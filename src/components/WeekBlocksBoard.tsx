...
export const WeekBlocksBoard = ({
  ...
  const todoistTasksGroupedByCategory = useMemo(() => {
    const grouped = new Map<string, Map<string, TodoistTaskOption>>();
    todoistTasks.forEach((task) => {
      const groups = task.groups.length > 0 ? task.groups : ["Uncategorized"];
      groups.forEach((groupName) => {
        const key = groupName.trim() || "Uncategorized";
        const current = grouped.get(key) ?? new Map<string, TodoistTaskOption>();
    ...
    return [...grouped.entries()]
      ...
      .map(([category, tasksById]) => ({
        category,
        tasks: [...tasksById.values()].sort((a, b) => a.content.localeCompare(b.content)),
      }));
  }, [todoistTasks]);
  ...
  const selectTodoistCategoryTasks = (tasks: TodoistTaskOption[]) => {
    setSelectedTodoistTaskIds((previous) => {
      const next = new Set(previous);
      tasks.forEach((task) => {
        if (!knownTodoistExternalIds.has(task.id)) {
          next.add(task.id);
        }
      });
      return [...next];
      ...
      {todoistPickerOpen ? (
        <div className="mb-2 rounded-xl border border-rose-200 bg-white/85 p-2">
          ...
          <div className="max-h-64 space-y-2 overflow-y-auto pr-1">
            {todoistTasksGroupedByCategory.length === 0 ? (
              ...
              todoistTasksGroupedByCategory.map((group) => (
                <div
                  key={group.category}
                  className="rounded-lg border border-rose-200 bg-rose-50/45 p-2"
                >
                  <div className="mb-1 flex items-center justify-between gap-2">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.06em] text-rose-900/80">
                      {group.category}
                    </p>
                    <button
                      type="button"
                      onClick={() => selectTodoistCategoryTasks(group.tasks)}
                      className="rounded-md border border-rose-200 bg-white px-2 py-1 text-[10px] font-semibold text-rose-900 hover:bg-rose-50"
                    >
                      Select category
                    </button>
                  </div>
